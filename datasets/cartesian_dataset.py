...
from models.utils import to_tensor,FFT2D
...
class MRIDataset_Cartesian(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # =======
        T2_256_img_k = T2_256_img.permute(1, 2, 0).contiguous()
        T2_256_img_k_ks = torch.view_as_real(FFT2D(torch.view_as_complex(T2_256_img_k))).contiguous()
        ...
        T2_128_img_real = (T2_128_img_real + 1) / 2
        T2_128_img_imag = (T2_128_img_imag + 1) / 2
        ...
        T1_128_real = (T1_128_real + 1) / 2
        T1_128_imag = (T1_128_imag + 1) / 2
...
# ---------------------over------
        return {'ref_image_full': T1_256_img,
                'ref_image_sub': T1_128_img, #  if 2x, change it to T1_128_img
                'tag_image_full': T2_256_img,
                'tag_kspace_full':T2_256_img_ks,
                'tag_image_sub': T2_128_img, # if 2x, change it to T2_128_img
